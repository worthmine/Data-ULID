...
my $dt     = ulid_date( $ulid, dummy => 'ignored', time_zone => 'Asia/Tokyo' );

ok( $dt->isa("DateTime"), "ulid_date() yields DateTime" );

my $b_dt = ulid_date( $b_ulid, UTC => 'isIgnored', time_zone => 'Asia/Tokyo' );

ok( "$dt" eq "$b_dt", "Canonical and binary ULID yield same DateTime" );

my $o_dt = ulid_date( $old_ulid, 'Asia/Tokyo' => 'extra', time_zone => 'Asia/Tokyo' );
